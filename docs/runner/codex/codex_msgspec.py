...
try:
    ...
    _MSGSPEC_IMPORT_ERROR = exc
else:
    _MSGSPEC_IMPORT_ERROR = None


def _require_msgspec() -> Any:
    if msgspec is None:
        raise RuntimeError(
            "msgspec is required to use codex_msgspec; install msgspec first"
        ) from _MSGSPEC_IMPORT_ERROR
    return msgspec


if msgspec is None:
    ThreadEvent: TypeAlias = Any

    def decode_event(_data: bytes | str) -> Any:
        _require_msgspec()
